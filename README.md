...
- **Not production-hardened.** Single-node everything, no automated test suite yet (the checks
  above are scripts, not unit/integration tests), and evaluation runs on local fixtures. Tracked
  in `improvements.txt`.
...
improvements.txt   phased hardening roadmap
```
